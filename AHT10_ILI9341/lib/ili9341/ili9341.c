...
uint16_t ili9341_pinCS = 17;
uint16_t ili9341_pinDC = 20;
int16_t ili9341_pinRST = 16;

uint16_t ili9341_pinSCK = 18;
uint16_t ili9341_pinTX = 19;
...
void LCD_deinitDisplay(void) {
    // 1. Desliga o periférico SPI
    spi_deinit(ili9341_spi);

    // 2. Coloca os pinos em um estado seguro e conhecido
    //    Isso evita que fiquem "flutuando" e causando ruído.
    
    // CS (Chip Select) deve ficar em ALTO (inativo)
    gpio_init(ili9341_pinCS);
    gpio_set_dir(ili9341_pinCS, GPIO_OUT);
    gpio_put(ili9341_pinCS, 1);

    // RST (Reset) deve ficar em ALTO (operacional)
    if (ili9341_pinRST != -1) {
        gpio_init(ili9341_pinRST);
        gpio_set_dir(ili9341_pinRST, GPIO_OUT);
        gpio_put(ili9341_pinRST, 1);
    ...
    // Os outros pinos podem ser apenas inicializados como GPIO padrão.
    gpio_init(ili9341_pinDC);
    gpio_init(ili9341_pinSCK);
    gpio_init(ili9341_pinTX);
}
...
void LCD_fillScreen(uint16_t color) {
    ILI9341_Select();
    LCD_setAddrWindow(0, 0, _width, _height);
    ...
    // Buffer temporário (pode ser maior se quiser mais velocidade)
    uint16_t buffer[64];
    ...
    while (total > 0) {
        ...
        spi_write16_blocking(ili9341_spi, buffer, chunk);
        total -= chunk;
    }
